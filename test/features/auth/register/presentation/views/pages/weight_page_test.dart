...
void main() {
  ...
  Widget buildTestableWidget() {
    return BlocProvider<RegisterViewModel>.value(
      value: mockViewModel,
      child: MaterialApp(
        home: Scaffold(body: WeightPage(pageController: pageController)),
      ),
    );
  }

  testWidgets('WeightPage renders all UI elements correctly', (tester) async {
  ...
  testWidgets('WeightPage shows default weight 90', (tester) async {
    await tester.binding.setSurfaceSize(const Size(1080, 2400));
  ...
  testWidgets('WeightPage changes weight when scrolling to a different value', (
    tester,
  ) async {
    await tester.binding.setSurfaceSize(const Size(1080, 2400));
  ...
  testWidgets('WeightPage arrow up button increases weight', (tester) async {
    await tester.binding.setSurfaceSize(const Size(1080, 2400));
